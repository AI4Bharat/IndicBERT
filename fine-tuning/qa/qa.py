...
if args.do_train:
    ...
    def zero_shot(dataset):
        ...
        print(f"Results for {args.eval_data}: {compute_metrics(start_logits_d, end_logits_d, val_dataset, dataset['validation'])}")
    ...
    if 'indicqa' in args.eval_data:
        eval_dataset = load_dataset("ai4bharat/IndicQA", f"{args.eval_data}", use_auth_token=True)
    elif 'squad' in args.eval_data:
        eval_dataset = load_dataset("squad")

    zero_shot(eval_dataset)